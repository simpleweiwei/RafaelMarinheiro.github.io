...
You can see more results [here](/projects/photometric_stereo/calibrated_results).
...
We can use Linear Least Squares to solve this overconstrained problem. After that, we form the $$B$$ matrix, then we compute its SVD $$B = U\Sigma V^\top$$ and then we set $$A = $$
